        ...
        <div class="checkout-section mt-16 mb-16 flex justify-between bg-white p-6 rounded-xl shadow-lg">
            <div class="w-[48%]">
                ...
                <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <div class= "pr-4">
                            <button id="orderDetailsButton" type="button"
                                class="w-full bg-blue-500 text-white py-2 px-4 mb-2">
                                catatan
                            </button>
                        </div>
            ...
            <div class="w-[48%]">
                <table class="w-full border-2 border-black">
                    ...
                    <tbody class="checkout-details">
                        ...
                        <tr>
                            ...
                            <td class="border-2 border-black"><span id="ongkir">Pilih alamat dulu</span></td>
                        </tr>
                        <tr>
                            <td class="border-2 border-black">Total</td>
                            <td class="border-2 border-black"><span id="total">Rp.
                ...
                @if ($daftar_alamats->count() > 0)
                    ...
                    <button type="submit"
                        class="bg-green-500 text-white p-2 my-2 rounded-lg hidden" id="conditionalElement">PESAN
                        SEKARANG</button>
                @endif
            </div>
            </form>
...
@section('script')
    <script>
        ...
        $('#daftar_alamat_id').on('change', function(e) {
            ...
            const inp = $('#layanan')
            const ongkir = $('#ongkir')
            const input_ongkir = $('#input_ongkir')
            ongkir.html('Loading');
            ...
            $.ajax({
                ...
                success: function({
                    ...
                    const firstService = data.costs[0];

                    inp.append(new Option(firstService.service + ' | ' + firstService.description +
                        ' | ' + firstService.cost[0].value, firstService.service));

                    ongkir.html('Rp. ' + number_format(firstService.cost[0].value));
                    input_ongkir.val(firstService.cost[0].value); // Use .val() to set the input value
                    // Update the total span with the sum of $total and ongkir
                    const newTotal = parseInt(total) + parseInt(firstService.cost[0].value);

                    const conditionalElement = document.getElementById('conditionalElement');
                    if (parseInt(firstService.cost[0].value) !== 0) {
                        conditionalElement.classList.remove('hidden');
                    } else {
                        conditionalElement.classList.add('hidden');
                    }

                    // Update the #total span directly using JavaScript
                    document.getElementById('total').innerText = 'Rp. ' + newTotal.toString().replace(
                        /\B(?=(\d{3})+(?!\d))/g, ",");
                    // inp.on('change', function(e) {
                    //     document.querySelector('#ongkir').value = data.cost[0].value;
                    //     document.querySelector('[name="estimasi"]').value = firstService.cost[0]
                    //         .etd;
                    //     document.querySelector('#total').innerText = 'Rp. ' + (parseInt(total) +
                    //         parseInt(firstService.cost[0].value)).toString().replace(
                    //         /\B(?=(\d{3})+(?!\d))/g, ",");
                    // });
